...
/** Adds a scalar into a 4D vector. */
KZ_INLINE void kzcVector4ComponentAdd(const struct KzcVector4* first, kzFloat second, struct KzcVector4* targetVector)
{
    targetVector->data[0] = first->data[0] + second;
    targetVector->data[1] = first->data[1] + second;
    targetVector->data[2] = first->data[2] + second;
    targetVector->data[3] = first->data[3] + second;
}
...
/** Subtracts a scalar from a 4D vector. */
KZ_INLINE void kzcVector4ComponentSubtract(const struct KzcVector4* first, kzFloat second, struct KzcVector4* targetVector)
{
    targetVector->data[0] = first->data[0] - second;
    targetVector->data[1] = first->data[1] - second;
    targetVector->data[2] = first->data[2] - second;
...
/** Component-modulo of a 4D vectors and a scalar. */
KZ_INLINE void kzcVector4ModuloScalar(const struct KzcVector4* first, kzFloat second, struct KzcVector4* targetVector)
{
    targetVector->data[0] = kzsFmod(first->data[0], second);
    targetVector->data[1] = kzsFmod(first->data[1], second);
    targetVector->data[2] = kzsFmod(first->data[2], second);
    targetVector->data[3] = kzsFmod(first->data[3], second);
}
...
KZ_INLINE void kzcVector4ComponentPow(const struct KzcVector4* vector, kzFloat exponent, struct KzcVector4* out_vector)
{
    ...
    out_vector->data[3] = kzsPowf(vector->data[3], exponent);
}
