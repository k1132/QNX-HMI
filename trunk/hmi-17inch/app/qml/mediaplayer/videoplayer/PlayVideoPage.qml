...
Item{
    ...
    function showFullScreen() {
        _mediaPlayerModule.videoPlayer.setViewPort(0, 88, 1200, 784)
    }

    // Hides the video window
    function hideViewPort() {
        _mediaPlayerModule.videoPlayer.setViewPort(192, 1078, -1, -1)
        _mediaPlayerModule.videoPlayer.pause()
    }

    function updateViewPort() {
        if(root.effectivelyVisible) {
            if(root.playingFullScreenVideo) {
                video_fullscreen.currentColumn = 1
                showFullScreen();
            } else {
    ...
    Component.onCompleted: {
        ...
        video_progressbar.value = root.mediaSourceSelected ? (100 * video_progressbar.left_time / video_progressbar.right_time) : 0
        //_mediaPlayerModule.workflow.enterPlayEmbeddedVideoState();
    }
...
Item {
    ...
    visible: !root.playingFullScreenVideo

    Image {
        id: backgroundImage

        anchors.fill: parent
        source: Util.fromTheme("MediaControl/video/background.png")
    }
    ...
    MultiStateButton{
        ...
        currentColumn: root.playingFullScreenVideo ? 1 : 0
        onBtn_Released: {
            /* if video is playing **/
            if(root.mediaSourceSelected){
    ...
    ProgressSlider{
        ...
        property bool playingState: root.mediaSourceSelected
        ...
        roundAngle: false
        onLeft_timeChanged: {
             value = root.mediaSourceSelected ? (100 * _mediaPlayerModule.videoPlayer.position / right_time) : 0
        }
    ...
    Rectangle{
        ...
        Text{
            ...
            text: root.mediaSourceSelected ? _mediaPlayerModule.videoTitle : "No Movie"
