    ...
    , m_audioTrackOffset(0)
{
    ...
    m_ppsObject = new QPps::Object(QStringLiteral("/pps/hinge-tech/media"), QPps::Object::PublishAndSubscribeMode, true, this);
   ...
   // m_browseModel->browse(0 + m_audioTrackOffset, QStringLiteral("/"));
...
void MediaPlayerModule::ppsAttributeChanged(const QString &name, const QPps::Variant &attribute)
{
    ...
    else if (name == QString::fromLatin1(pps_kanziVideoClickedKey))
    {
        ...
        if (access(QString(QString::fromLatin1("/accounts/1000/shared/videos/%1.mp4")).arg(m_videoClickedId).toLatin1().data(), F_OK) == 0)
        {
            ...
            playVideoForce(0 + m_audioTrackOffset , QString(QString::fromLatin1("/accounts/1000/shared/videos/%1.mp4")).arg(m_videoClickedId));
        }
    }
}
...
int MediaPlayerModule::circleMode()
{
    return m_circleMode;
}

void MediaPlayerModule::setCircleMode(int mode)
{
    m_circleMode = mode;
}
...
bool MediaPlayerModule::musicFileExist()
{
    QString tmps = QStringLiteral("/accounts/1000/shared/music");
    QDir dir(tmps);
    QStringList filter;
    filter<<QStringLiteral("*.mp3")<<QStringLiteral("*.aac")<<QStringLiteral("*.cda")<< QStringLiteral("*.m4b")<< QStringLiteral("*.wav");
...
void MediaPlayerModule::onVideoUrlLoadedForce(QPlayer::ExtendedMetadataCommand *command)
{
    ...
    if(command->result().contains(urlKey)) {
        //        std::cout << "===============" << "video url = " << command->result().value(urlKey).toUrl().toString().toLatin1().data() << std::endl;
        videoPlayer()->openUrlForce(command->result().value(urlKey).toUrl());
        //        std::cout << "===============" << "video pixel width = " << videoPlayer()->videoPixelWidth() << std::endl;
        //        std::cout << "===============" << "video pixel height = " << videoPlayer()->videoPixelHeight() << std::endl;
    } else {
