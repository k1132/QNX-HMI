...
{
    m_ppsControlObject = new QPps::Object(QStringLiteral("/pps/hinge-tech/radio/control"),
                                          QPps::Object::PublishMode, true, this);
    m_ppsStatusObject = new QPps::Object(QStringLiteral("/pps/hinge-tech/radio/status"),
                                         QPps::Object::SubscribeMode, true, this);
    m_ppsTunersObject = new QPps::Object(QStringLiteral("/pps/hinge-tech/radio/tuners"),
                                         QPps::Object::SubscribeMode, true, this);
    ...
    if (m_ppsStatusObject->isValid()) {
        ...
        m_amStation = m_ppsStatusObject->attribute(QString::fromLatin1("am_station")).toDouble();
        attributeToStations(m_ppsStatusObject->attribute(QString::fromLatin1(pps_fmKey)), m_presetFmStations, m_fmStation);
        m_fmStation = m_ppsStatusObject->attribute(QString::fromLatin1("fm_station")).toDouble();
...
void Radio::Private::simulateSeek(bool upDirection)
{
    ...
    const double rangeStep = tunerInfo[QStringLiteral("rangeSvoid Radio::Private::ppsStatusAttributeChanged(const QString &name, const QPps::Variant &attribute)tep")].toDouble();
...
void Radio::Private::ppsStatusAttributeChanged(const QString &name, const QPps::Variant &attribute)
{
    if (name == QString::fromLatin1(pps_amKey)) {
        ...
        station = m_ppsStatusObject->attribute(QString::fromLatin1("am_station")).toDouble();
        //printf("am station is %f\n",station);
        ...
        attributeToStations(attribute, presetStations, station);
        station = m_ppsStatusObject->attribute(QString::fromLatin1("fm_station")).toDouble();
        //printf("fm station is %f\n",station);
    ...
    } else if (name == QString::fromLatin1(pps_amstationKey)) {
//        if (m_stationName != attribute.toString()) {
//            m_stationName = attribute.toString();
//            emit q->stationNameChanged(m_stationName);
//        }
          if(m_amStation != attribute.toDouble())
          {
              m_amStation = attribute.toDouble();
              emit q->stationChanged(m_amStation);
          }
    }
    else if(name == QString::fromLatin1(pps_fmstationKey))
    {
        if(m_fmStation != attribute.toDouble())
        {
            m_fmStation = attribute.toDouble();
            emit q->stationChanged(m_fmStation);
        }
    }
...
bool Radio::audios() const
{
    return d->m_IsAudioOn;
}

void Radio::setAudios(bool audios)
{
    if(audios)
    {
        if (!d->m_ppsControlObject->setAttribute(QStringLiteral("audio"), QPps::Variant(QStringLiteral("on"))))
        {
            qWarning() << "Radio: unable to write back: audio on";
            return;
        }
        d->m_IsAudioOn = true;
    }
    ...
    {
        if (!d->m_ppsControlObject->setAttribute(QStringLiteral("audio"), QPps::Variant(QStringLiteral("off"))))
        {
            qWarning() << "Radio: unable to write back: audio off";
            return;
        }
         d->m_IsAudioOn = false;
    }
...
void Radio::tune(double rawStation)
{
    ...
    if(rawStation <rangeMin || rawStation>rangeMax)
    {
        return;
    }
    const double station  = rawStation;
    if (d->m_simulationModeEnabled){
        d->simulateTune(station);
    }
    else
    {
        if(d->m_tuner == AmTuner)
            d->m_ppsControlObject->setAttribute(QStringLiteral("am_station"), QPps::Variant(station));
        if(d->m_tuner == FmTuner)
            d->m_ppsControlObject->setAttribute(QStringLiteral("fm_station"), QPps::Variant(station));
    }
}
